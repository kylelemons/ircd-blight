...
func newModeSpec(r rune, typ modeType, name string) *ModeSpec {
	var prefix string
	if typ == StatusMode {
		if idx := strings.IndexRune(statusMode, r); idx >= 0 {
			prefix = statusPrefix
		}
	}
	return &ModeSpec{
...
type ModeMap map[rune]*ModeSpec
...
type ActiveModes map[rune]Mode
...
func (am ActiveModes) Get(r rune) (isset bool, args []string) {
	var m Mode
	if m, isset = am[r]; isset {
		args = m.Args
	}
	return
}
...
func ModeString(modes []Mode) string {
	...
	putch := func(r rune) {
		modebytes = append(modebytes, byte(r))
	}
...
type UnknownModeError struct{ Char rune }
type MissingArgumentError struct{ Char rune }
type UnsetMatchError struct{ Char rune }
