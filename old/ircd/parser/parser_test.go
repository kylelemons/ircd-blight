		...
		if len(test.args) != len(m.Args) {
			...
			for j := 0; j < len(test.args) && j < len(m.Args); j++ {
				if test.args[j] != m.Args[j] {
					t.Errorf("%d: Expected arg %q, got %q", i, test.args[j], m.Args[j])
				}
			}
		}
	}
...
func TestBuildMessage(t *testing.T) {
	for i, test := range build_message_tests {
		...
		if test.expected != str {
			t.Errorf("%d: Expected string representation %q, got %q", i, test.expected, str)
		}
		if string(bytes) != str {
			t.Errorf("%d: Expected identical string and byte representation, got %q and %q", i, bytes, str)
		}
	}
}
